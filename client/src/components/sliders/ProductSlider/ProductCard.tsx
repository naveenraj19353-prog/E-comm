...
const ProductCard = ({ product, isWishlisted = false, onWishlist, onAddToCart, isAdding = false, }: ProductCardProps) => {
    ...
    const handleAddToCart = (event: React.MouseEvent<HTMLButtonElement>) => {
        ...
        const variant = (product.inventory || []).find((item) => Number(item.stock) > 0 && item.variantId);
        if (!variant) {
            return;
        }
    ...
    return (<div className={styles.card} onClick={handleCardClick} role="link" tabIndex={0} onKeyDown={(event) => {
      ...
      <div className={styles.content}>
        ...
        <div className={styles.price}>
          ...
          {product.price > product.finalPrice && (<span className={styles.old}>
              ₹{product.price.toLocaleString()}
            </span>)}
        </div>
        <button type="button" className={styles.cartBtn} onClick={handleAddToCart} disabled={outOfStock || isAdding}>

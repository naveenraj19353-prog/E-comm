...
const FilterDrawer = ({ open, onClose, children, onClear, }: FilterDrawerProps) => {
    return (<>
      <div className={`${styles.overlay} ${open ? styles.overlayVisible : ""}`} onClick={onClose} aria-hidden="true"/>
      <aside className={`${styles.drawer} ${open ? styles.drawerOpen : ""}`} aria-hidden={!open} aria-label="Product filters">

...
export default function CreateProduct() {
   ...
   *
   * We remember which color the user is uploading images for.
   */
  ...
  const [colorImageNames, setColorImageNames] =
    useState<ColorImageNames>({});
  ...
  const handleAddColor = () => {
    ...
    setNewColor("");
    /*
     * Create inventory rows for all existing sizes.
     */
  ...
  const handleAddSize = () => {
    ...
    setNewSize("");
    /*
     * Create inventory rows for all existing colors.
     */
    setInventory((previous) => {
      const updated = [...previous];
  ...
  const handleChooseImages = (
    ...
    /*
     * Open file selector after setting color.
     */
    setTimeout(() => {
  ...
  const handleSubmit = async (
    ...
    for (const item of inventory) {
      ...
      if (
        ...
        setError(
          "Every inventory variant must have a color and size.",
        );
        return;
      }
      if (
        ...
        setError(
          `Enter a valid stock for ${item.color} - ${item.size}.`,
        );
        return;
      }
    }
    /*
     * Make sure every color-size combination
    ...
    for (const color of colors) {
      for (const size of sizes) {
        ...
        if (!row) {
          setError(
            `Missing inventory for ${color} - ${size}.`,
          );
          return;
        }
      }
     ...
     * Validate images.
     *
     * Every color must have at least one image.
     */
    for (const color of colors) {
      if (
        !colorImages[color] ||
    ...
    /*
     * Convert inventory to backend format.
     */
    const inventoryPayload = inventory.map(
      (item) => ({
    ...
    try {
      await createProductMutation.mutateAsync({
        ...
        discountPercentage:
          Number(discountPercentage) || 0,
        /*
         * Keep flat stock compatibility.
         *
         * Backend can also calculate totalStock.
         */
        stock: inventoryPayload.reduce(
          (total, item) =>
            total + item.stock,
          0,
        ),
        /*
         * Keep sizes/colors available
          ...
          <div className={styles.grid}>
            ...
            <div className={styles.field}>
              ...
              <input
                ...
                placeholder="Example: WOMEN-FASHION"
              />
              <small>
                ...
                (color) => {
                  ...
                  return (
                    <div
                      key={color}
                      className={
                        styles.colorImageSection
                      }
                    >
                      <div
                        className={
                          styles.colorImageHeader
                        }
                      >
                        <div>
                          <h3>

...
export default function ProductsPage() {
    ...
    const tenantIdStore = useAppSelector((state) => state.tenant.tenantSlug) ?? "";
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                setLoading(true);
                setError("");
                const response = await apiClient.get<ProductsResponse>("/product/get-all-products", {
                    params: {
                        tenantId,
                        page,
                        limit,
    ...
    return (<div className={styles.page}>
      <div className={styles.header}>
        ...
        <button type="button" className={styles.createButton} onClick={() => navigate("/admin/products/create")}>
          <span>+</span>
          Add Product
        </button>
      </div>
      <div className={styles.toolbar}>
        ...
        <select className={styles.select} value={tenantId} onChange={(event) => {
      ...
      <div className={styles.tableCard}>
        ...
        {loading ? (<div className={styles.state}>Loading products...</div>) : error ? (<div className={styles.stateError}>{error}</div>) : products.length === 0 ? (<div className={styles.empty}>
                ...
                {products.map((product) => (<tr key={product._id}>
                    ...
                    <td>
                      <div className={styles.price}>
                        ...
                        {product.discountPercentage > 0 && (<span>{formatPrice(product.price)}</span>)}
                      </div>
                    </td>
                    <td>
                      <span className={product.totalStock === 0 || product.stock === 0
                    ? styles.outOfStock
                    ...
                    <td>
                      <button type="button" className={styles.editButton} onClick={() => navigate(`/admin/products/${product._id}/edit`)}>
                        Edit
                      </button>
                    </td>
                  </tr>))}
        ...
        {!loading && products.length > 0 && (<div className={styles.pagination}>
            <button type="button" disabled={page === 1} onClick={() => setPage((current) => Math.max(1, current - 1))}>
              ← Previous
            </button>
            <span>Page {page}</span>
            <button type="button" disabled={products.length < limit} onClick={() => setPage((current) => current + 1)}>
              Next →
            </button>
          </div>)}
      </div>

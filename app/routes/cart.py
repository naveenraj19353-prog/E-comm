...
def add_to_cart(request: AddCart):
    ...
    if existing:
        ...
        carts.update_one(
            {"_id": existing["_id"]},
            {"$set": {"quantity": new_quantity, "updatedAt": datetime.utcnow()}},
        )
...
def update_cart(productId: str, request: UpdateCart):
    ...
    product = products.find_one(
        {"_id": ObjectId(productId), "tenantId": request.tenantId, "isActive": True}
    )
    ...
    carts.update_one(
        {"_id": cart["_id"]},
        {"$set": {"quantity": request.quantity, "updatedAt": datetime.utcnow()}},
    )
...
def remove_from_cart(productId: str, userId: str, tenantId: str):
...
def clear_cart(userId: str, tenantId: str):
    ...
    result = carts.delete_many({"tenantId": tenantId, "userId": ObjectId(userId)})
